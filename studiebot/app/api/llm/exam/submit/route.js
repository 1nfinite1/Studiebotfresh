...
export async function POST(req) {
  try {
    ...
    // Optional summary + focus points (lightweight placeholders, UI-safe)
    const summary = `Je haalde ${pct}% op ${total} vragen. Goed bezig — kijk vooral naar de uitleg bij de items. 🎯`;

    const headers = new Headers({ 'X-Debug': 'llm:exam|grade|used_material', 'X-Context-Size': String(lastCtxSize || 0) });
    return ok({
