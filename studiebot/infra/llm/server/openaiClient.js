...
async function checkRelevance(_client, _text) { return 'on_topic'; }

async function runGuardrailChecks(client, text) {
  if (!text) return { passed: true };
  if (detectPromptInjection(text)) return { passed: false, reason: 'prompt_injection' };
...
function attachUsageFields(target, resp, fallbackModel) {
  try {
    const usage = resp?.usage || {};
    target.model = resp?.model || fallbackModel || null;
    target.usage = {
    ...
    target.model = fallbackModel || null;
    target.usage = { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 };
  }
}
...
export async function srvGenerateHints({ topicId, text, currentBloom = 'remember', currentDifficulty = 'easy', wasCorrect = null, subject, grade, chapter }) {
  ...
  try {
    if (!c) {
      return { hints: ['(stub) Benoem 2-3 kernbegrippen uit de tekst.'], tutor_message: '(stub) Laten we de hoofdpunten kort doornemen.', follow_up_question: '(stub) Wat vind je het lastigste stukje?', defined_terms: [], next_bloom: currentBloom, next_difficulty: currentDifficulty, header: 'disabled', policy: { guardrail_triggered: false, reason: 'none' }, db_ok: true, context_len: (ctx.text || '').length, model: null, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } };
    }
    const guardrailCheck = await runGuardrailChecks(c, text);
    const response = { hints: [], tutor_message: '', follow_up_question: '', defined_terms: [], next_bloom: currentBloom, next_difficulty: currentDifficulty, header: 'enabled', policy: { guardrail_triggered: false, reason: 'none' }, db_ok: true, context_len: (ctx.text || '').length, model: null, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } };
    ...
    return response;
  } catch { return { hints: ['Er ging iets mis.'], tutor_message: 'Fout opgetreden.', follow_up_question: 'Kun je je vraag opnieuw stellen?', defined_terms: [], next_bloom: 'remember', next_difficulty: 'easy', notice: 'server_error', header: 'disabled', policy: { guardrail_triggered: false, reason: 'none' }, db_ok: false, context_len: 0, model: null, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } }; }
}
...
export async function srvQuizGenerate({ topicId, objective, currentBloom = 'remember', currentDifficulty = 'easy', subject, grade, chapter }) {
  const c = getClient();
  const ctx = await getContext({ subject, grade, chapter, topicId });
  if (!ctx.ok) { return { no_material: true, reason: ctx.reason, message: ctx.message, db_ok: ctx.db_ok, policy: { guardrail_triggered: false, reason: 'none' }, context_len: 0 }; }
  try {
    if (!c) { return { question_id: 'stub-q1', type: 'mcq', stem: '(stub) Korte vraag bij de tekst.', choices: ['A','B','C','D'], answer_key: { correct: [0], explanation: '(stub) Uitleg' }, objective: objective || 'algemeen', bloom_level: currentBloom, difficulty: currentDifficulty, source_ids: [], hint: null, defined_terms: [], header: 'disabled', policy: { guardrail_triggered: false, reason: 'none' }, db_ok: true, context_len: (ctx.text || '').length, model: null, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } }; }
    ...
    } catch { response.stem = 'Noem één belangrijk punt uit de tekst.'; response.type = 'short_answer'; response.choices = []; response.answer_key = { correct: [], explanation: 'Geef een beknopt antwoord.' }; }
    return response;
  } catch { return { question_id: `error-${Date.now()}`, type: 'short_answer', stem: 'Er ging iets mis.', choices: [], answer_key: { correct: [], explanation: 'Probeer opnieuw.' }, objective: objective || 'general', bloom_level: currentBloom, difficulty: currentDifficulty, source_ids: [], hint: null, defined_terms: [], notice: 'server_error', header: 'disabled', policy: { guardrail_triggered: false, reason: 'none' }, db_ok: false, context_len: 0, model: null, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } }; }
}
...
export async function srvGradeQuiz({ answers, questions = [], objectives = [], isExam = false, subject, grade, chapter, topicId }) {
  ...
  try {
    ...
    const response = { is_correct: false, score: 0.0, feedback: '', tags: [], next_recommended_focus: [], weak_areas: [], chat_prefill: '', header: 'enabled', policy: { guardrail_triggered: false, reason: 'none' }, db_ok: true, context_len: (ctx.text || '').length, model: null, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } };
    if (!guardrailCheck.passed) { response.feedback = GUARDRAIL_MESSAGE; response.policy = sanitizeGuardrail(response, ['feedback'], guardrailCheck.reason); return response; }
    ...
    try {
      ...
      attachUsageFields(response, resp, MODELS.grade);
      const content = resp.choices?.[0]?.message?.content || '{}';
      const json = JSON.parse(content);
      ...
      response.chat_prefill = json.chat_prefill || (weakParts.length > 0 ? `Ik heb moeite met ${weakParts.slice(0, 2).join(' en ')}. Ik wil daarop oefenen.` : 'Ik wil meer oefenen met deze stof.');
    } catch { response.score = 0.5; response.feedback = 'Goed geprobeerd! Probeer het nog eens met meer details.'; response.next_recommended_focus = ['Herhaal de hoofdpunten', 'Oefen met voorbeelden']; response.chat_prefill = 'Ik wil meer oefenen met deze stof.'; }
    return response;
  } catch { return { is_correct: false, score: 0.0, feedback: 'Er ging iets mis.', tags: [], next_recommended_focus: [], weak_areas: [], chat_prefill: '', notice: 'server_error', header: 'disabled', policy: { guardrail_triggered: false, reason: 'none' }, db_ok: false, context_len: 0, model: null, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } }; }
}
...
export async function srvExamGenerate({ topicId, blueprint = {}, totalQuestions = 5, subject, grade, chapter }) {
  ...
  if (!ctx.ok) { return { no_material: true, reason: ctx.reason, message: ctx.message, db_ok: ctx.db_ok, policy: { guardrail_triggered: false, reason: 'none' }, context_len: 0 }; }
  if (!c) {
    return { questions: Array.from({ length: totalQuestions }, (_, i) => ({ question_id: `stub-q${i + 1}`, type: 'mcq', stem: `(stub) Vraag ${i + 1}`, choices: ['A','B','C','D'], answer_key: { correct: [0], explanation: '(stub) Uitleg' }, objective: `objective-${i + 1}`, bloom_level: i < 2 ? 'remember' : i < 4 ? 'understand' : 'apply', difficulty: 'medium', source_ids: [], hint: null, defined_terms: [] })), blueprint: { by_objective: { OB1: 2, OB2: 2, OB3: 1 }, by_level: { remember: 2, understand: 2, apply: 1 } }, notice: 'LLM not configured', header: 'disabled', policy: { guardrail_triggered: false, reason: 'none' }, db_ok: true, context_len: (ctx.text || '').length, model: null, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } };
  }
  const response = { questions: [], blueprint: { by_objective: {}, by_level: { remember: 0, understand: 0, apply: 0 } }, header: 'enabled', policy: { guardrail_triggered: false, reason: 'none' }, db_ok: true, context_len: (ctx.text || '').length, model: null, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } };
  ...
  try {
    ...
    attachUsageFields(response, resp, MODELS.exam);
    const content = resp.choices?.[0]?.message?.content || '{}';
    const json = JSON.parse(content);
  ...
  } catch { response.questions = Array.from({ length: Math.min(totalQuestions, 3) }, (_, i) => ({ question_id: `fallback-q${i + 1}`, type: 'short_answer', stem: `Leg kort uit wat je weet over dit onderwerp.`, choices: [], answer_key: { correct: ['uitleg'], explanation: 'Geef een volledig antwoord.' }, objective: 'general', bloom_level: 'understand', difficulty: 'medium', source_ids: [], hint: null, defined_terms: [] })); }
  return response;
}

// Grade a single exam item and return short rubric + model answer; include model/usage
export async function srvExamGradeItem({ question, answer, subject, grade, chapter, topicId }) {
  const c = getClient();
  const ctx = await getContext({ subject, grade, chapter, topicId });
  if (!ctx.ok) { return { no_material: true, reason: ctx.reason, message: ctx.message, db_ok: ctx.db_ok, context_len: 0 }; }
  if (!c) {
    return { is_correct: false, score: 0.6, explanation: 'Voorbeeldbeoordeling; voeg later een LLM toe. 😊', model_answer: 'Kernpunten volgens de stof, kort samengevat.', header: 'disabled', db_ok: true, context_len: (ctx.text || '').length, model: null, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } };
  }
  const guardrailCheck = await runGuardrailChecks(c, String(answer || ''));
  if (!guardrailCheck.passed) { return { is_correct: false, score: 0.0, explanation: GUARDRAIL_MESSAGE, model_answer: '', header: 'enabled', db_ok: true, context_len: (ctx.text || '').length, model: null, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } }; }
  try {
    const { buildExamSystemGrade, buildExamUserGrade } = await import('../prompts');
    const system = buildExamSystemGrade();
    const user = buildExamUserGrade(ctx.text || '', String(question || ''), String(answer || ''));
    const resp = await c.chat.completions.create({ model: MODELS.grade, temperature: 0.3, response_format: { type: 'json_object' }, messages: [{ role: 'system', content: system }, { role: 'user', content: user }] });
    const content = resp.choices?.[0]?.message?.content || '{}';
    const json = JSON.parse(content);
    ...
    return { is_correct: false, score: 0.5, explanation: 'Goede poging! Dit kan nog preciezer. ✨', model_answer: 'Geef de kernbegrippen en 1 voorbeeld in 1–2 zinnen.', header: 'enabled', db_ok: true, context_len: (ctx.text || '').length, model: MODELS.grade, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } };
  }
}